...
shinyServer(
  ...
  function(input, output, session) {
    ...
    output$text <- renderText({
      choices = c("Bandt-Pompe", "Bandt-Pompe Weight", "Ordinal patterns transition graphs", "Weighted Ordinal Patterns Transition Graph")
    ...
    dataEntropy <- reactive({
      ...
      choices.dist = c("Bandt-Pompe", "Bandt-Pompe Weight", "Ordinal patterns transition graphs", "Weighted Ordinal Patterns Transition Graph")
    ...
    buildPlane <- reactive({
      ...
      if(option.plot == 1)
        p = HC.Plane(prob, d, ranges2$x, ranges2$y)
      else
        p = FS.Plane(prob, d, ranges2$x, ranges2$y)
    ...
    buildTimeSeries <- reactive({
      ...
      if(input$pattern > 0 && input$pattern <= factorial(input$dimension)){
        p = patterns.on.graph(series, input$dimension, input$delay, input$pattern, ranges$x, ranges$y)
      }
      if(input$pattern == 0){
        p = time.series.plane(series, ranges$x, ranges$y)
      }
    ...
    buildHistogram <- reactive({
      ...
      series = matrix(unlist(series), ncol = length(series))
      series = series[,which(input$chosen.series == series.name)]
      p = histogram(series, input$dimension, input$delay)
      p
